...
const startMarkers = /[<>v^]/;
const [sx, sy] = (() => {
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const c = rows[y][x];
            if (startMarkers.test(c))
                return [x, y];
        }
    }
...
// returns [x, y, dindex]
function getJumpDestination(x, y, dindex) {
    // wall conditional
    if (rows[y][x] === "#")
        return null;
...
function jumpToWall(dindex, wallLoc) {
...
function jumpSim(x, y, dindex, wallLoc) {
    var _a;
    const goal = (_a = jumpMap.get(`${x},${y},${dindex}`)) !== null && _a !== void 0 ? _a : null;
    if (wallLoc && goal) {
        const [fx, fy, _] = goal;
        const [wx, wy] = wallLoc;
...
function pathLoopsWithPatch(wallLoc) {
